    ...
    <div>
      ...
      >
        <el-tabs v-model="activeTab" type="card" @tab-click="handleTabChange">
          <el-tab-pane
            :label="`粉丝${$page.allFollowers.pageInfo.totalItems}`"
            name="followers"
            style="padding: 5px"
          >
            <div v-if="$page.allFollowers.edges.length">
              <el-row style="min-height: 200px; ">
                <el-col
                  :span="8"
                  v-for="item in $page.allFollowers.edges"
                  :key="item.node.id"
                  style="padding: 10px"
                >
                  <el-card
                  ...
                  >
                    ...
                    <a
                      @click="$router.push(`/user/${item.node.name}`)"
          ...
          <el-tab-pane
            :label="`关注${$page.allFollowing.pageInfo.totalItems}`"
            name="following"
            style="padding: 5px"
          >
            <div v-if="$page.allFollowing.edges.length">
              <el-row style="min-height: 200px; ">
                <el-col
                  :span="8"
                  v-for="item in $page.allFollowing.edges"
                  :key="item.node.id"
                  style="padding: 10px"
                >
                  <el-card
