  ...
  async created() {
    ...
    try {
      const res = await request(
        `/repos/${config.USERNAME}/${name}/contents/README.md`
      )

      const md = new MarkdownIt()
      data.content = md.render(Base64.decode(res.data.content))
    } catch (e) {
      data.content = ''
    } finally {
      this.loading = false
    ...
    const createTime = this.changeTime(data.created_at)
    const updateTime = this.changeTime(data.updated_at)
    this.project = { name, createTime, updateTime, ...data }
  },
  methods: {
    goGithub(url) {
      window.open(url)
